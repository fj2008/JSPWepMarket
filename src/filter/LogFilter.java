...
public class LogFilter implements Filter{
	...
	public void destroy() {
		...
		writer.close();//이 필터가 끝날때 받아주는 코드
	}
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		try {
			writer = new PrintWriter(new FileWriter(filename,true), true);//스트림을 문자기반스트림을 적용함
		}catch(IOException e) {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		long start = System.currentTimeMillis();//이 필터가 사용된 시간
		...
		long end = System.currentTimeMillis();//이필터가 response받은 시간
	...
	private String getURLPath(ServletRequest request) {
		...
		if(request instanceof HttpServletRequest) {
			...
			queryString = req.getQueryString();//get방식으로보낸 요청파라미터만 가지고옴
