		...
		try {
			...
			while(rs.next()) {
				...
				Product product = new Product(p_id, p_name, p_unitPrice,p_description,p_manufacturer,
								p_category,p_unitsInStock,p_condition,p_imagePath);//n번째 상품의 데이터를 저장할 객체 생성
				
				productList.add(product);//위 자료를 리스트에 저장
			}//end while
